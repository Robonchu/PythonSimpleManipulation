...
def make_a_poses():
    ...
    Y_OFFSET = 0.0
    target_poses = []
    target_poses.append(np.array([0.2, 0.08, UP_HEIGHT]))
    target_poses.append(np.array([0.2, 0.08, DOWN_HEIGHT]))
    target_poses.append(np.array([0.2, 0.01, DOWN_HEIGHT]))
    target_poses.append(np.array([0.17, 0.04, DOWN_HEIGHT]))
    target_poses.append(np.array([0.19, 0.05, UP_HEIGHT]))
    target_poses.append(np.array([0.19, 0.05, DOWN_HEIGHT]))
    target_poses.append(np.array([0.16, 0.06, DOWN_HEIGHT]))
    target_poses.append(np.array([0.13, 0.08, DOWN_HEIGHT]))
    target_poses = np.array(target_poses)
    target_poses += np.array([X_OFFSET, Y_OFFSET, 0])
    return target_poses
...
def make_ke_poses():
    ...
    X_OFFSET = -0.02
    Y_OFFSET = 0.05
    target_poses.append(np.array([0.2, -0.01, UP_HEIGHT]))
    target_poses.append(np.array([0.2, -0.01, DOWN_HEIGHT]))
    target_poses.append(np.array([0.18, -0.005, DOWN_HEIGHT]))
    target_poses.append(np.array([0.19, -0.01, UP_HEIGHT]))
    target_poses.append(np.array([0.19, -0.01, DOWN_HEIGHT]))
    target_poses.append(np.array([0.19, -0.03, DOWN_HEIGHT]))
    target_poses.append(np.array([0.19, -0.06, DOWN_HEIGHT]))
    target_poses.append(np.array([0.19, -0.06, UP_HEIGHT]))
    target_poses.append(np.array([0.19, -0.03, DOWN_HEIGHT]))
    target_poses.append(np.array([0.16, -0.03, DOWN_HEIGHT]))
    target_poses.append(np.array([0.14, -0.01, DOWN_HEIGHT]))
    target_poses = np.array(target_poses)
    target_poses += np.array([X_OFFSET, Y_OFFSET, 0])
    return target_poses
...
def make_o_poses():
    ...
    X_OFFSET = 0.05
    Y_OFFSET = 0.0
    target_poses.append(np.array([0.16, -0.01, UP_HEIGHT]))
    target_poses.append(np.array([0.16, -0.01, DOWN_HEIGHT]))
    target_poses.append(np.array([0.16, -0.03, DOWN_HEIGHT]))
    target_poses.append(np.array([0.16, -0.06, DOWN_HEIGHT]))
    target_poses.append(np.array([0.18, -0.04, UP_HEIGHT]))
    target_poses.append(np.array([0.18, -0.04, DOWN_HEIGHT]))
    target_poses.append(np.array([0.12, -0.04, DOWN_HEIGHT]))
    target_poses.append(np.array([0.16, -0.04, UP_HEIGHT]))
    target_poses.append(np.array([0.16, -0.04, DOWN_HEIGHT]))
    target_poses.append(np.array([0.13, -0.01, DOWN_HEIGHT]))
    target_poses = np.array(target_poses)
    target_poses += np.array([X_OFFSET, Y_OFFSET, 0])
    return target_poses
...
def make_me_poses():
    ...
    target_poses.append(np.array([0.16, 0.01, UP_HEIGHT]))
    target_poses.append(np.array([0.16, 0.01, DOWN_HEIGHT]))
    target_poses.append(np.array([0.12, -0.02, DOWN_HEIGHT]))
    target_poses.append(np.array([0.15, -0.025, UP_HEIGHT]))
    target_poses.append(np.array([0.15, -0.025, DOWN_HEIGHT]))
    target_poses.append(np.array([0.12, 0.01, DOWN_HEIGHT]))
    target_poses.append(np.array([0.115, 0.03, DOWN_HEIGHT]))
    target_poses = np.array(target_poses)
    target_poses += np.array([X_OFFSET, Y_OFFSET, 0])
    return target_poses


def make_akeome_poses():
    a = make_a_poses()
    ke = make_ke_poses()
    o = make_o_poses()
    me = make_me_poses()
    akeome = np.concatenate([a, ke, o, me])
    # akeome = make_o_poses()
    # akeome = make_me_poses()
    # a.extend(make_ke_poses())
    # a.extend(make_o_poses())
    # a.extend(make_me_poses())    # a = make_a_poses()
    # ke = make_ke_poses()
    # o = make_o_poses()
    # me = make_me_poses()
    # akeome = np.concatenate([a, ke, o])
    # a = make_a_poses()
    # ke = make_ke_poses()
    # o = make_o_poses()
    # me = make_me_poses()
...
if __name__ == "__main__":
    ...
    # target_poses = np.array(make_a_poses())
    # target_poses = np.array(make_ke_poses())
    target_poses = np.array(make_akeome_poses())

    # please remove the below on real experiment
    # target_poses = target_poses[np.where(target_poses[:, 2] != UP_HEIGHT)]
    ...
    interpolated_angles = AngleInterpolation(angles, 2)
    # import pdb
    # pdb.set_trace()
    angle_list = angles[-1]
    '''
    ...
    # fig = plt.figure(figsize=(14, 1), dpi=80)
